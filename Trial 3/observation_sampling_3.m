...
sample_interval = linspace(1,50,30);
observationspan = linspace(201,400,40);
test_iter = 10;
...
for m = 1:numel(sample_interval) 
    n = 1;
    meanF1(m) = 0;
    while (meanF1(m) < .9) && (observationspan(n) < observationspan(end))
        for l = 1:test_iter
            ...
            train_data = [C_healthy_noise1_train(1:round(sample_interval(m)):(round(observationspan(n))+round(sample_interval(m))),:,m,l),C_unhealthy_noise1_train(1:round(sample_interval(m)):(round(observationspan(n))+round(sample_interval(m))),:,m,l)];
            test_data = [C_healthy_noise1_test(1:round(sample_interval(m)):(round(observationspan(n))+round(sample_interval(m))),:,m,l),C_unhealthy_noise1_test(1:round(sample_interval(m)):(round(observationspan(n))+round(sample_interval(m))),:,m,l)];
    ...
    observ_t(m) = observationspan(x(m)) + sample_interval(m);% check
