...
for m = 1:size(k_gr_agg_rnd,2)
    for n = 1:size(k_gr_non_rnd,2)
        for l = 1:test_iter
            ...
            C_healthy_noise1_test(:,:,n,m,l) = C_healthy_noise1(:,squeeze(~tf_healthy2(:,n,m,l)),m);
            % for now using the same index of random selection for healthy and
            % unhealthy
            C_unhealthy_noise1_train(:,:,n,m,l) = C_unhealthy_noise1(:,squeeze(tf_healthy2(:,n,m,l)),n);
...
figure;
for i = 3:3%size(k_gr_agg_rnd,2)
    h = plot(m_k_gr_non_rnd, meanF1(:,end-1),'*','MarkerSize',15);
    hold on
end
