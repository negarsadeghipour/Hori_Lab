...
s_mean_k_growth_agg = linspace(0,0.02,15);
...
k_decay_agg_rnd = (max_k_decay_agg - min_k_decay_agg).*rand(num_patients,size(k_gr_agg_rnd,2)) + min_k_decay_agg;%linspace(1/150,1/30,50); %day-1
...
k_decay_non_rnd = (max_k_decay_non - min_k_decay_non).*rand(num_patients,size(k_gr_agg_rnd,2)) + min_k_decay_non;%mean([1/(24*30) 1/150]);
k_gr_non_rnd = (max_k_gr_non - min_k_gr_non).*rand(num_patients,size(k_gr_agg_rnd,2)) + min_k_gr_non;%mean([0 1/18/30]);
...
C_healthy_noise1 = zeros(numel(time),num_patients,size(k_gr_agg_rnd,2));
C_unhealthy_noise1 = zeros(numel(time),num_patients,size(k_gr_agg_rnd,2));
...
test_iter = 10;
tf_healthy = false(N,size(k_gr_agg_rnd,2),test_iter);    % create logical index vector
tf_healthy(1:round(p*N),:,:) = true;


for n = 1:size(k_gr_agg_rnd,2)
    for l = 1:test_iter
     ...
     meanF1(n) = mean(mean(f1Scores(:,n,:),1),3);
end
...
figure;
h = plot(m_k_gr_agg_rnd, squeeze(meanF1),'*','MarkerSize',15);
title('k_{Growth} Aggressive');
xlabel('k_{Growth} Aggressive (day^{-1})'); ylabel('f1 score');
set(gca,'box','off','TickDir','out',...
