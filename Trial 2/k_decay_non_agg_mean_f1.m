...
min_k_gr_non = 1e-5;
s_mean_k_decay_non = linspace(0,.1,15);
...
k_gr_non_rnd = (max_k_gr_non - min_k_gr_non).*rand(num_patients,size(k_decay_non_rnd,2)) + min_k_gr_non;%mean([0 1/18/30]);
...
k_gr_agg_rnd = (max_k_gr_agg - min_k_gr_agg).*rand(num_patients,size(k_decay_non_rnd,2)) + min_k_gr_agg;%linspace(1/18/30,1/2/30,50); %day-1
k_decay_agg_rnd = (max_k_decay_agg - min_k_decay_agg).*rand(num_patients,size(k_decay_non_rnd,2)) + min_k_decay_agg;%linspace(1/150,1/30,50); %day-1
...
for k = 1:size(k_decay_non_rnd,2)
...
test_iter = 10;
tf_healthy = false(N,size(k_decay_non_rnd,2),test_iter);    % create logical index vector
tf_healthy(1:round(p*N),:,:) = true;


for n = 1:size(k_decay_non_rnd,2)
    for l = 1:test_iter
...
figure;
h = plot(m_k_decay_non_rnd, squeeze(meanF1),'*','MarkerSize',15);
title('k_{Decay} Non-aggressive');
xlabel('k_{Decay} Non-aggressive (day^{-1})'); ylabel('f1 score');
set(gca,'box','off','TickDir','out',...
